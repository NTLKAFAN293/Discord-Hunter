...
export default function Home() {
  const [settings, setSettings] = useState<CheckSettings>(defaultSettings);
  ...
  const runCheckLoop = useCallback(async () => {
    ...
    let retries = 3;
    while (result.error === "Rate limited" && retries > 0) {
      toast({
        title: "Rate limited",
        description: `Retrying in ${settings.delayMs / 1000} seconds... (${retries} retries left)`,
        variant: "warning",
      });
